...
describe('wrap-queries', function () {
  ...
  it('passes though stylesheets with existing @media query', function (done) {
    var mediaCSS = process.cwd() + '/test/fixtures/existingmedia.css';
    ...
    vinyl.src(mediaCSS)
      .pipe(wrap())
      .pipe(gutil.buffer(function (err, files) {
        expect(path.basename(files[0].path)).to.equal('existingmedia.css');
  ...
  it('passes though stylesheets with existing @keyframes query', function (done) {
    var keyframesCSS = process.cwd() + '/test/fixtures/existingkeyframes.css';
    ...
    vinyl.src(keyframesCSS)
      .pipe(wrap())
      .pipe(gutil.buffer(function (err, files) {
        expect(path.basename(files[0].path)).to.equal('existingkeyframes.css');
